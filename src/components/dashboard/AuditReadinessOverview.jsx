...
export default function AuditReadinessOverview({ records = [], summary }) {
  ...
  const data = summary || Object.entries(groups).map(([name, scores]) => ({ name, score: scores.reduce((a, b) => a + b, 0) / scores.length })).sort((a, b) => b.score - a.score);
  return (
